...
const CAUTION = "Practical service guidance, Tg, HDT, Vicat softening, melting point, and other named tests answer different questions. Compare only matching metric and method groups.";
...
function factState(fact: MapDisplayFact): string {
  switch (fact.state) {
    ...
    case "missing": return "Not reported";
  }
}
...
function disposition(record: Readonly<{ disposition: MapServiceGuidanceRecord["disposition"] }>): string {
...
function evidenceScopes(record: Readonly<{ evidence: MapServiceGuidanceRecord["evidence"] }>): string {
  return record.evidence.scopeLabels.length === 0 ? "Not reported" : record.evidence.scopeLabels.join("; ");
}

function qualification(record: Readonly<{ evidence: MapServiceGuidanceRecord["evidence"] }>): string {
  return record.evidence.qualification ?? "No separate qualification is reported.";
}

function selectThermalMaterial(dispatch: MapDispatch, materialId: MapServiceGuidanceRecord["material"]["id"]): void {
  dispatch({ type: "select-material", mode: "thermal-ranges", materialId });
}

function previewHandlers(dispatch: MapDispatch, materialId: MapServiceGuidanceRecord["material"]["id"]) {
  const target = { kind: "material" as const, mode: "thermal-ranges" as const, id: materialId };
  return {
    onFocus: () => dispatch({ type: "preview-selection", mode: "thermal-ranges", source: "focus", target }),
    onBlur: () => dispatch({ type: "clear-preview", mode: "thermal-ranges", source: "focus" }),
    onMouseEnter: () => dispatch({ type: "preview-selection", mode: "thermal-ranges", source: "hover", target }),
    onMouseLeave: () => dispatch({ type: "clear-preview", mode: "thermal-ranges", source: "hover" }),
  };
}

function ThermalControls({ view, dispatch }: Readonly<{ view: MapViewModel; dispatch: MapDispatch }>) {
  ...
  return (
    <form class="map-controls thermal-controls" onSubmit={(event: JSX.TargetedSubmitEvent<HTMLFormElement>) => event.preventDefault()}>
      <fieldset>
        ...
        <label>
          <input
            ...
            onChange={(event: JSX.TargetedEvent<HTMLInputElement>) => setView("service-guidance", event.currentTarget.checked)}
          />
        ...
        <label>
          <input
            ...
            onChange={(event: JSX.TargetedEvent<HTMLInputElement>) => setView("named-observations", event.currentTarget.checked)}
          />
      ...
      <label>
        ...
        <input
          ...
          value={thermal.query}
          onInput={(event: JSX.TargetedInputEvent<HTMLInputElement>) => dispatch({
            type: "set-search", target: "thermal", query: event.currentTarget.value,
          })}
        />
      </label>
        ...
        <label>
          ...
          >
            ...
            {thermal.groups.map((group) => (
              <option key={group.id} value={group.id}>{group.metricLabel} — {group.methodLabel}</option>
            ))}
          </select>
        </label>
      )}
      ...
      >Clear thermal filters</button>
    </form>
  );
}

function ServiceDiagram({ view, dispatch }: Readonly<{ view: MapViewModel; dispatch: MapDispatch }>) {
  ...
  const position = (value: number) => left + ((value - domain.low) / (domain.high - domain.low)) * plotWidth;
  return (
    <svg
    ...
    >
      {ticks.map((tick) => (
        <g key={tick}>
          <line class="thermal-axis-grid" x1={position(tick)} y1="32" x2={position(tick)} y2={height - 12} />
          <text x={position(tick)} y="22" text-anchor="middle">{tick} °C</text>
        </g>
      ))}
      {serviceRecords.map((record, index) => {
        ...
        return (
          ...
          >
            <text x={left - 12} y={y + 5} text-anchor="end">{record.material.name}</text>
            {low !== undefined && high !== undefined && (
              <>
                {selected && <line class="thermal-service-mark__selection" x1={position(low) - 4} y1={y} x2={position(high) + 4} y2={y} />}
                <line
                  class={`thermal-service-mark__interval${record.fact.state === "conditional" ? " is-conditional" : ""}`}
                  x1={position(low)} y1={y} x2={position(high)} y2={y}
                />
                <line class="thermal-service-mark__cap" x1={position(low)} y1={y - 7} x2={position(low)} y2={y + 7} />
                <line class="thermal-service-mark__cap" x1={position(high)} y1={y - 7} x2={position(high)} y2={y + 7} />
              </>
            )}
          </g>
        );
...
function ServiceSelected({ record, methodHref }: Readonly<{
  ...
  return (
    <section class="map-selected-record" aria-labelledby="selected-service-heading" aria-live="off">
      ...
      <dl>
        <div><dt>State</dt><dd>{factState(record.fact)}</dd></div>
        <div><dt>Value</dt><dd>{measurementValue(record.measurement)} {measurementUnit(record.measurement)}</dd></div>
        <div><dt>Guidance</dt><dd>{SERVICE_LABEL}</dd></div>
        <div><dt>Qualification</dt><dd>{qualification(record)}</dd></div>
        <div><dt>Evidence scope</dt><dd>{evidenceScopes(record)}</dd></div>
        <div><dt>Diagram state</dt><dd>{disposition(record)}</dd></div>
      </dl>
      <p><a href={record.material.href}>Open material reference</a></p>
      <p><a href={methodHref}>Review method and thermal definitions</a></p>
    </section>
  );
}
...
function ServiceView({ view, dispatch, methodHref }: Props) {
  ...
  const filtered = records.filter(({ disposition: state }) => state.disposition === "filtered").length;
  return (
    <div class="thermal-view thermal-view--service">
      <p class="map-current-state" aria-live="off">{plotted} records plotted; {filtered} filtered from the diagram; {omitted.length} not plotted.</p>
      <div class="map-legend" aria-label="Practical service guidance legend">
        <span><span class="map-mark map-mark--service-interval" aria-hidden="true"></span> Practical service interval</span>
      ...
      <div class="map-horizontal-scroll" role="region" aria-label="Practical service guidance diagram" tabIndex={0}>
        <p class="map-scroll-instruction">Scroll horizontally to inspect the Celsius axis and all interval endpoints.</p>
      ...
      {omitted.length > 0 && (
        <section class="map-omissions" aria-labelledby="service-omissions-heading">
          ...
          <ul>{omitted.map((record) => <li key={record.material.id}><a href={record.material.href}>{record.material.name}</a>: {disposition(record)}</li>)}</ul>
        </section>
      )}
      <div class="map-horizontal-scroll" role="region" aria-label="Complete practical service guidance table" tabIndex={0}>
        ...
        <table class="thermal-table">
          ...
          <tbody>{records.map((record) => (
            <tr data-service-row={true} key={record.material.id}>
              ...
              <td>{factState(record.fact)} — {disposition(record)}</td>
              <td>{measurementValue(record.measurement)}</td>
              <td>{measurementUnit(record.measurement)}</td>
              <td>{SERVICE_LABEL}</td>
              <td><a href={methodHref}>Review defined method and conditions</a></td>
...
function NamedShape({ member, x, y }: Readonly<{ member: MapThermalMember; x: number; y: number }>) {
  ...
  if (member.metric === "heat-deflection") return <rect class={className} x={x - 7} y={y - 7} width="14" height="14" />;
  if (member.metric === "vicat-softening") return <polygon class={className} points={`${x},${y - 9} ${x + 9},${y} ${x},${y + 9} ${x - 9},${y}`} />;
  if (member.metric === "melting-point") return <polygon class={className} points={`${x},${y - 9} ${x + 9},${y + 8} ${x - 9},${y + 8}`} />;
  return <path class={className} d={`M ${x - 7} ${y - 7} L ${x + 7} ${y + 7} M ${x + 7} ${y - 7} L ${x - 7} ${y + 7}`} />;
}
      ...
      {members.map((member, index) => {
        ...
        return (
          ...
          >
            ...
            <text x="206" y={y + 5}>{member.material.name}: {member.fact.display.join(" — ")}</text>
          </g>
        );
      })}
    </svg>
...
function NamedSelected({ record, methodHref }: Readonly<{ record: MapNamedThermalRecord; methodHref: MapInternalHref }>) {
  ...
  return (
    <section class="map-selected-record" aria-labelledby="selected-named-heading" aria-live="off">
      ...
      {member === undefined ? <p>No observation in this metric and method group.</p> : (
        <dl>
          <div><dt>State</dt><dd>{factState(member.fact)}</dd></div>
          <div><dt>Value</dt><dd>{measurementValue(member.measurement)} {measurementUnit(member.measurement)}</dd></div>
          <div><dt>Metric</dt><dd>{member.metricLabel}</dd></div>
          <div><dt>Method and conditions</dt><dd>{member.methodLabel}</dd></div>
          <div><dt>Qualification</dt><dd>{qualification(member)}</dd></div>
          <div><dt>Evidence scope</dt><dd>{evidenceScopes(member)}</dd></div>
        </dl>
      )}
      <p><a href={record.material.href}>Open material reference</a></p>
      <p><a href={methodHref}>Review method and thermal definitions</a></p>
...
function NamedView({ view, dispatch, methodHref }: Props) {
  ...
  if (group === undefined) {
    return (
      <section class="map-recovery" aria-labelledby="named-group-required-heading">
        <h3 id="named-group-required-heading">Choose a named metric and method group to inspect its records.</h3>
  ...
  return (
    <div class="thermal-view thermal-view--named">
      ...
      <div class="map-horizontal-scroll" role="region" aria-label={`${group.metricLabel} named observation diagram`} tabIndex={0}>
        <p class="map-scroll-instruction">Scroll horizontally to inspect complete observation labels.</p>
      ...
      {omitted.length > 0 && (
        <section class="map-omissions" aria-labelledby="named-omissions-heading">
          ...
          <ul>{omitted.map((record) => (
            <li key={record.material.id}><a href={record.material.href}>{record.material.name}</a>: No observation in this metric and method group</li>
          ))}</ul>
        </section>
      )}
      <div class="map-horizontal-scroll" role="region" aria-label="Complete named thermal observation table" tabIndex={0}>
        <p class="map-scroll-instruction">Scroll horizontally to inspect every column.</p>
        <table class="thermal-table">
          <caption>{group.metricLabel} — {group.methodLabel}; every material is retained</caption>
          <thead><tr>
            <th scope="col">Material</th><th scope="col">State</th><th scope="col">Value</th><th scope="col">Unit</th>
            <th scope="col">Metric or guidance</th><th scope="col">Method and conditions</th><th scope="col">Qualification</th>
            <th scope="col">Evidence scope</th><th scope="col">Material reference</th>
          ...
          <tbody>{records.map((record) => {
            ...
            return (
              <tr data-named-row={true} key={record.material.id}>
                <th scope="row">{record.material.name}</th>
                <td>{member === undefined ? "No observation in this metric and method group" : factState(member.fact)}</td>
  ...
  return (
    <section class="map-mode map-mode--thermal" aria-labelledby="interactive-thermal-heading">
      <header class="map-mode__header">
        ...
        <p class="map-caution"><strong>Compare defined concepts only.</strong> {CAUTION}</p>
      </header>
