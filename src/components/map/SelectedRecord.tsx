...
export function SelectedRecord(props: SelectedRecordProps) {
  if (props.kind === "process-lane") {
    return (
      <section
        class="map-selected-record"
        aria-labelledby="selected-process-lane-heading"
        aria-live="off"
      >
        <p class="technical-eyebrow">Selected decision lane</p>
        <h3 id="selected-process-lane-heading">{props.lane.label}</h3>
        <p>
          <strong>Need:</strong> {props.lane.need}
        </p>
        <p>
          <strong>Selected relationship state:</strong> Selected
        </p>
          ...
          <dl>
            {props.gates.map((gate) => (
              <div key={gate.id}>
                ...
                <dd>
                  Applies — verify this gate. {gate.requirement} {gate.verification}
                </dd>
              </div>
            ))}
          </dl>
  ...
  if (props.kind === "process-gate") {
    return (
      <section
        class="map-selected-record"
        aria-labelledby="selected-process-gate-heading"
        aria-live="off"
      >
        <p class="technical-eyebrow">Selected process gate</p>
        <h3 id="selected-process-gate-heading">
          <a href={props.gate.href}>{props.gate.label}</a>
        </h3>
        <dl>
          <div>
            <dt>Capability category</dt>
            <dd>{props.gate.capabilityLabel}</dd>
          </div>
          <div>
            <dt>Requirement</dt>
            <dd>{props.gate.requirement}</dd>
          </div>
          <div>
            <dt>Verification guidance</dt>
            <dd>{props.gate.verification}</dd>
          </div>
          <div>
            <dt>Relationship meaning</dt>
            <dd>Applies — verify this gate. This is not a printer capability verdict.</dd>
          </div>
        </dl>
        <h4>Referenced lanes and their live candidates</h4>
        {props.lanes.length === 0 ? (
          <p>No decision lane lists this process gate.</p>
        ) : (
  ...
  return (
    <section
      class="map-selected-record"
      aria-labelledby="selected-impact-flex-heading"
      aria-live="off"
    >
      <p class="technical-eyebrow">Selected material</p>
      <h3 id="selected-impact-flex-heading">{record.material.name}</h3>
      {props.outsideFilter && (
        <p class="map-state-label">Selected record is outside the current diagram filter.</p>
      )}
      <dl>
          ...
          <dd>
            {record.disposition.disposition === "plotted"
              ...
              : record.disposition.disposition === "filtered"
                ...
                : `Not plotted — ${record.disposition.reason}`}
          </dd>
        </div>
      </dl>
      ...
      {props.evidenceHref === undefined ? (
        ...
        <p>
          <a href={props.evidenceHref}>Review method and evidence conventions</a>
        </p>
      )}
    </section>
  );
