...
export function ImpactFlexMatrix({ view, dispatch, evidenceHref }: Props) {
  ...
  const active =
    model.activeTarget?.mode === "impact-flex-space" && model.activeTarget.kind === "material"
      ? model.activeTarget
      : undefined;
  const locked =
    model.lockedTarget?.mode === "impact-flex-space" && model.lockedTarget.kind === "material"
      ? model.lockedTarget
      : undefined;
  const highlighted =
    active === undefined
  ...
  const filtered = model.records.filter(
    ({ disposition }) => disposition.disposition === "filtered",
  );
  ...
  const selectMaterial = (record: MapImpactFlexRecord) =>
    dispatch({
  ...
  return (
    <section
      id="impact-flex-space"
      class="map-mode map-mode--impact-flex"
      aria-labelledby="interactive-impact-flex-heading"
    >
      <header class="map-mode__header">
        <p class="technical-eyebrow">Impact and flexibility</p>
        <h2 id="interactive-impact-flex-heading">
          Inspect two ordered qualitative categories together
        </h2>
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event: JSX.TargetedEvent<HTMLSelectElement>) => {
              const term = model.difficultyTerms.find(
                ({ value }) => value === event.currentTarget.value,
              );
              dispatch({
                type: "set-maximum-difficulty",
                ...(term === undefined ? {} : { value: term.value }),
              });
            }}
          >
            <option value="">Any print difficulty</option>
            {model.difficultyTerms.map((term) => (
        ...
        <button
          type="button"
          onClick={() => dispatch({ type: "reset-view", mode: "impact-flex-space" })}
        >
      ...
      <p class="map-current-state">
        {plotted.length} plotted; {filtered.length} filtered from the diagram; {omitted.length} not
        plotted. All {model.records.length} records remain in the table.
      </p>

      <div class="map-legend" aria-label="Impact-flex mark legend">
        <span>
          <span class="map-mark map-mark--open" aria-hidden="true"></span> Material
        </span>
        <span>
        ...
        {model.shapesEnabled &&
          model.difficultyTerms.map((term) => (
            <span key={term.value}>
              <span class={`map-mark map-mark--${term.shape}`} aria-hidden="true"></span>
              {term.label}
            </span>
          ))}
      </div>
      ...
      <div
        ...
        role="region"
        aria-label="Impact resistance by flexibility matrix"
        tabIndex={0}
      >
        <p class="map-scroll-instruction">
          Scroll horizontally to inspect all category combinations.
        </p>
        <figure class="impact-flex-figure">
          <svg
            viewBox="0 0 900 520"
            role="img"
            aria-labelledby="impact-flex-title impact-flex-description"
            focusable="false"
          >
            <title id="impact-flex-title">Impact resistance by flexibility category matrix</title>
            <desc id="impact-flex-description">
              Twenty categorical cells. Interactive marks duplicate the complete ordered material
              controls and table below.
            </desc>
            <text x="506" y="506" text-anchor="middle">
              Impact resistance
            </text>
            <text x="24" y="242" text-anchor="middle" transform="rotate(-90 24 242)">
              Flexibility
            </text>
            {model.impactAxis.map((term) => (
              <text
                key={term.value}
                x={PLOT.left + term.order * PLOT.column + PLOT.column / 2}
                y="492"
                text-anchor="middle"
              >
                {term.label}
            ...
            {[...model.flexibilityAxis].reverse().map((term, row) => (
              <text
                key={term.value}
                x="164"
                y={PLOT.top + row * PLOT.row + PLOT.row / 2}
                text-anchor="end"
              >
                {term.label}
              </text>
            ...
            {plotted.map((record) => {
              ...
              const conditional =
                record.impactFact.state === "conditional" ||
                record.flexibilityFact.state === "conditional";
              return (
                <g
                  ...
                  onPointerDown={(event: JSX.TargetedPointerEvent<SVGGElement>) =>
                    event.preventDefault()
                  }
                  onClick={() => selectMaterial(record)}
                >
                  {isSelected && (
                    <circle class="impact-flex-mark__selection" cx={x} cy={y} r={13} />
                  )}
                  <Shape
                    shape={model.shapesEnabled ? (record.shape ?? "circle") : "circle"}
                    x={x}
                    y={y}
                    className="impact-flex-mark__shape"
                  />
                </g>
              );
            })}
          ...
          <figcaption>
            Cell spacing indicates category order only. Cell numbers report currently plotted
            records.
          </figcaption>
        </figure>
      </div>
          ...
          {model.records.map((record) => (
            <li key={record.material.id}>
              <button
                ...
                data-previewed={
                  highlighted?.material.id === record.material.id &&
                ...
                onBlur={() =>
                  dispatch({ type: "clear-preview", mode: "impact-flex-space", source: "focus" })
                }
                onClick={() => selectMaterial(record)}
              >
      ...
      <div
        class="map-horizontal-scroll"
        role="region"
        aria-label="Complete impact and flexibility material table"
        tabIndex={0}
      >
        <p class="map-scroll-instruction">Scroll horizontally to inspect every column.</p>
        <table class="impact-flex-table">
          <caption>
            All materials in categorical order, including filtered and unplottable records
          </caption>
          <thead>
            <tr>
          ...
          <tbody>
            {model.records.map((record) => (
              <tr data-impact-row="true" key={record.material.id}>
                ...
                <td>
                  <span
                    class={`map-fact map-fact--${record.impactFact.state}`}
                    aria-hidden="true"
                  ></span>
                  {record.impactFact.state}
                </td>
                <td>{visibleValue(record.flexibilityFact)}</td>
                <td>
                  <span
                    class={`map-fact map-fact--${record.flexibilityFact.state}`}
                    aria-hidden="true"
                  ></span>
                  {record.flexibilityFact.state}
                </td>
