...
export function DataControls({ model, state, onChange, onInvalid, onClear }: Props) {
  ...
  const updateGroup = (value: string) => {
    ...
    onChange((current) => ({ ...current, group: next.key, thermalMetric: "all", sort: { field: first.key as ExplorerSortableField, direction: "asc" } }));
  };
  ...
  const updateSort = (value: string) => {
    ...
    onChange((current) => ({ ...current, sort: { ...current.sort, field: field.key as ExplorerSortableField } }));
  };

  return (
    <form class="data-controls" onSubmit={(event: JSX.TargetedSubmitEvent<HTMLFormElement>) => event.preventDefault()}>
      <label><span>Search materials and visible values</span><input type="search" value={state.query} onInput={(event: JSX.TargetedEvent<HTMLInputElement>) => updateQuery(event.currentTarget.value)} /></label>
      ...
      <fieldset><legend>View</legend>{(["table", "records"] as const).map((view) => <label key={view}><input type="radio" name="explorer-view" value={view} checked={state.view === view} onChange={(event: JSX.TargetedEvent<HTMLInputElement>) => event.currentTarget.value === view ? onChange((current) => ({ ...current, view })) : onInvalid()} />{view === "table" ? "Table" : "Material records"}</label>)}</fieldset>
      <fieldset><legend>Sort direction</legend>{(["asc", "desc"] as const).map((direction) => <label key={direction}><input type="radio" name="sort-direction" value={direction} checked={state.sort.direction === direction} onChange={(event: JSX.TargetedEvent<HTMLInputElement>) => event.currentTarget.value === direction ? onChange((current) => ({ ...current, sort: { ...current.sort, direction } })) : onInvalid()} />{direction === "asc" ? "Ascending" : "Descending"}</label>)}</fieldset>
