...
export function defaultExplorerState(model: DataExplorerModel): ExplorerState {
  ...
  const firstSortable = group.fieldKeys
    .map((key) => model.fields.find((field) => field.key === key))
    .find((field) => field !== undefined && field.sort !== "none") ?? fail("EXPLORER_MODEL_INVALID");
  return deepFreeze({
    query: "",
    group: group.key,
    thermalMetric: "all",
...
function validateState(model: DataExplorerModel, input: unknown): ExplorerState {
  if (typeof input !== "object" || input === null || Array.isArray(input)) fail("EXPLORER_STATE_INVALID");
  const state = input as Record<string, unknown>;
  if (!hasExactKeys(state, ["query", "group", "thermalMetric", "factState", "evidenceScope", "view", "sort"])) {
    fail("EXPLORER_STATE_INVALID");
  }
  if (typeof state.query !== "string" || state.query.length > 500) fail("EXPLORER_STATE_INVALID");
  const group = model.groups.find(({ key }) => key === state.group) ?? fail("EXPLORER_STATE_INVALID");
  if (state.thermalMetric !== "all" && !model.thermalMetrics.some(({ id }) => id === state.thermalMetric)) {
    fail("EXPLORER_STATE_INVALID");
  }
  if (state.factState !== "all" && !["known", "conditional", "unknown", "missing", "not-applicable"].includes(String(state.factState))) {
    fail("EXPLORER_STATE_INVALID");
  }
  if (state.evidenceScope !== "all" && !EVIDENCE_SCOPE_ORDER.includes(state.evidenceScope as EvidenceScope)) {
    fail("EXPLORER_STATE_INVALID");
  }
  if (state.view !== "table" && state.view !== "records") fail("EXPLORER_STATE_INVALID");
  if (typeof state.sort !== "object" || state.sort === null || Array.isArray(state.sort)) fail("EXPLORER_STATE_INVALID");
  const sort = state.sort as Record<string, unknown>;
  if (!hasExactKeys(sort, ["field", "direction"]) || (sort.direction !== "asc" && sort.direction !== "desc")) {
    fail("EXPLORER_STATE_INVALID");
  }
  const field = model.fields.find(({ key }) => key === sort.field);
  if (field === undefined || !group.fieldKeys.includes(field.key) || field.sort === "none") fail("EXPLORER_STATE_INVALID");
  return {
...
function narrowThermal(cell: ExplorerThermalCell, groupId: string): ExplorerThermalCell {
  ...
  return {
    ...
    searchText: members.flatMap(({ metricLabel, methodLabel, display, qualification, scopeLabels }) =>
      [metricLabel, methodLabel, ...display, qualification, ...scopeLabels]
    ),
    sortKey: cell.key === "thermal-value"
...
function compareRows(
  ...
  const leftKnown = (leftCell.sortKey.state === "known" || leftCell.sortKey.state === "identity") && leftCell.sortKey.value !== undefined;
  const rightKnown = (rightCell.sortKey.state === "known" || rightCell.sortKey.state === "identity") && rightCell.sortKey.value !== undefined;
...
export function exploreData(model: DataExplorerModel, input: unknown): ExplorerSuccess {
  ...
  const group = model.groups.find(({ key }) => key === state.group) ?? fail("EXPLORER_MODEL_INVALID");
  const fields = group.fieldKeys.map((key) => model.fields.find((field) => field.key === key) ?? fail("EXPLORER_MODEL_INVALID"));

  const materials = model.materials.flatMap((material): readonly ExploredMaterial[] => {
    let cells = group.fieldKeys.map((key) => material.cells.find((cell) => cell.key === key) ?? fail("EXPLORER_MODEL_INVALID"));
    if (state.thermalMetric !== "all") {
      const hasMetric = cells.some((cell) => cell.kind === "thermal" && cell.members.some(({ groupId }) => groupId === state.thermalMetric));
      if (!hasMetric) return [];
      cells = cells.map((cell) => cell.kind === "thermal" ? narrowThermal(cell, state.thermalMetric) : cell);
    }
    if (state.query !== "") {
      const searchable = [material.name, material.family, ...cells.flatMap(({ searchText }) => searchText)]
