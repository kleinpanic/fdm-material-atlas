...
import { FACT_STATE_PRESENTATION, THERMAL_KIND_PRESENTATION } from "../../lib/presentation/labels.ts";
...
const FILTERS = [
  ["print-difficulty", "Print difficulty", "vocabulary-print-difficulty", (m: Material) => m.process.printDifficulty.value],
  ["enclosure", "Enclosure requirement", "vocabulary-enclosure-requirement", (m: Material) => m.process.enclosure.value],
  ["hardened-nozzle", "Wear-resistant nozzle requirement", "vocabulary-hardened-nozzle-requirement", (m: Material) => m.process.hardenedNozzle.value],
  ["drying-priority", "Drying priority", "vocabulary-drying-priority", (m: Material) => m.process.dryingPriority.value],
  ["ventilation", "Ventilation category", "vocabulary-ventilation-category", (m: Material) => m.process.ventilation.value],
  ["cost-tier", "Relative cost tier", "vocabulary-cost-tier", (m: Material) => m.costTier.value],
  ["outdoor-uv", "Outdoor and UV behavior", "vocabulary-outdoor-uv", (m: Material) => m.properties.outdoorUv.value],
  ["impact-resistance", "Impact resistance", "vocabulary-impact-resistance", (m: Material) => m.properties.impactResistance.value],
  ["flexibility", "Flexibility", "vocabulary-flexibility", (m: Material) => m.properties.flexibility.value],
  ["chemical-resistance", "Chemical resistance", "vocabulary-chemical-resistance", (m: Material) => m.properties.chemicalResistance.value],
  ["cooling-shrink-risk", "Cooling-shrink risk", "vocabulary-cooling-shrink-risk", (m: Material) => m.properties.coolingShrinkRisk.value],
  ["dimensional-stability", "Dimensional stability", "vocabulary-dimensional-stability", (m: Material) => m.properties.dimensionalStability.value],
] as const;

type ErrorCode = "ATLAS_PAGE_EMPTY" | "ATLAS_PAGE_MATERIAL_DUPLICATE" | "ATLAS_PAGE_SLUG_DUPLICATE" | "ATLAS_PAGE_VOCABULARY_INVALID" | "ATLAS_PAGE_OUTPUT_OVERSIZED";
...
function projectFact<T>(fact: FactState<T>, labels?: ReadonlyMap<string, string>, format: (value: T) => string = String): AtlasFact {
  const base = { state: fact.state, stateLabel: FACT_STATE_PRESENTATION[fact.state].label } as const;
  if (fact.state === "known") {
  ...
  if (fact.state === "conditional") {
    const projected = fact.value === undefined ? {} : (() => {
      const value = format(fact.value);
      if (labels && !labels.has(value)) fail("ATLAS_PAGE_VOCABULARY_INVALID");
  ...
  if (fact.state === "not-applicable") return { ...base, ...(fact.reason ? { reason: fact.reason } : {}) };
...
export function buildAtlasPageModel(atlas: AtlasV1, base: string | undefined): AtlasPageModel {
  ...
  const definitions = FILTERS.map(([id, label, vocabularyId]) => {
    ...
    return { id, label, options: vocabulary.terms.map((term) => ({ id: term.value, label: term.label, kind: "value" as const })) };
  });
  ...
  const rows = [...atlas.materials]
    ...
    .map((material): AtlasRow => {
      const facts = Object.fromEntries(FILTERS.map(([id, , vocabularyId, read]) => {
        const vocabulary = vocabularyById.get(vocabularyId);
        if (!vocabulary) fail("ATLAS_PAGE_VOCABULARY_INVALID");
      ...
      return {
        ...
        serviceTemperature: { ...service, ...(material.serviceTemperature.qualification ? { qualification: material.serviceTemperature.qualification } : {}) },
        thermalObservations: material.thermalObservations.map((observation) => ({
          metricLabel: THERMAL_KIND_PRESENTATION[observation.metric].label === "Other named metric" ? observation.metricLabel : THERMAL_KIND_PRESENTATION[observation.metric].label,
          measurement: projectFact(observation.measurement, undefined, measurementLabel),
          qualification: observation.qualification,
        })),
        facts,
        uses: material.guidance.bestSuitedFor.value.state === "known" ? material.guidance.bestSuitedFor.value.value.slice(0, 3) : [],
