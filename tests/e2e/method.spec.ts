...
test("method fragments explain evidence, thermal distinctions, scoring, profiles, and limitations", async ({ page }) => {
  await page.goto(`${basePath}method/`);
  for (const id of ["evidence-scopes", "thermal-metrics", "selector-scoring", "qualitative-guidance", "starting-profiles", "methods", "sources", "limitations"]) {
    await expect(page.locator(`#${id}`)).toHaveCount(1);
  }
  await expect(page.getByText("not directly interchangeable", { exact: false })).toBeVisible();
  for (const term of ["Tg", "HDT", "Vicat", "Melting point"]) await expect(page.getByText(term, { exact: false }).first()).toBeVisible();
  await expect(page.getByText("universal material quality", { exact: false })).toBeVisible();
  await expect(page.getByText("not an engineering safety certification", { exact: false })).toBeVisible();
});

test("source actions are isolated HTTPS links and supporting claims return to exact material anchors", async ({ page }) => {
  ...
  const supportingDetails = page.locator("#sources details").filter({ has: page.locator("a[href*='/materials/']") }).first();
