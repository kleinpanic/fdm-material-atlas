...
test("generated material reference exposes canonical identity and complete static specimens", async ({
  page,
}) => {
  ...
  await expect(page.locator(".scientific-state__glyph").first()).toHaveAttribute(
    "aria-hidden",
    "true",
  );
  await expect(page.locator(".scientific-state__glyph").first()).toHaveAttribute(
...
test("breadcrumb and return journeys stay within the selected deployment base", async ({
  page,
}) => {
  await openTracer(page);
  ...
  await expect(breadcrumb.getByRole("link", { name: "Materials" })).toHaveAttribute(
    "href",
    `${basePath}materials/`,
  );
  await expect(breadcrumb.getByText(materialName!, { exact: true })).toHaveAttribute(
    "aria-current",
    "page",
  );
  ...
  await expect(page.getByRole("heading", { level: 1 })).toHaveText(
    "Browse validated material families",
  );
});

test("material-reference meaning remains visible without hover, animation, or color", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("link", { name: "Return to the complete material atlas" }),
  ).toBeVisible();
});
