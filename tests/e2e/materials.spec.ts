...
test("the no-script atlas lists every canonical material and links to static references", async ({ browser }) => {
  const context = await browser.newContext({ javaScriptEnabled: false });
  const page = await context.newPage();
  await page.goto(`${basePath}materials/`);
  await expect(page.locator(".atlas-row")).toHaveCount(23);
  await expect(page.locator(".atlas-fallback")).toContainText("All validated materials remain listed");
  const hrefs = await page.locator(".atlas-row__action").evaluateAll((links: Element[]) => links.map((link: Element) => link.getAttribute("href")));
  expect([...hrefs].sort()).toEqual(slugs.map((slug) => `${basePath}materials/${slug}/`).sort());
  await context.close();
});

test("hydrated filters announce results, expose active state, and recover from no results", async ({ page }) => {
  await page.goto(`${basePath}materials/`);
  const search = page.getByLabel("Search material or family");
  await expect(search).toBeEnabled();
  await search.fill("no-material-has-this-name");
  await expect(page.getByRole("heading", { name: "No materials match these filters" })).toBeVisible();
