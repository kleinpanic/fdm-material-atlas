...
test("axe passes empty, invalid, two-material, and four-material comparison states", async ({
  page,
}) => {
...
test("axe passes default, zero-result, and record data explorer states", async ({ page }) => {
  ...
  await axePasses(page);
  await page
    .getByLabel("Search materials and visible values")
    .fill("no-public-material-matches-this-query");
...
test("keyboard focus and aggregate status remain stable through comparison and explorer updates", async ({
  page,
}) => {
...
test("320px and 200 percent zoom retain 44px controls, local table overflow, and reading order", async ({
  page,
}) => {
  for (const state of [
    { width: 320, zoom: "100%" },
    ...
    const compareTargets = page
      .getByRole("form", { name: "Choose materials to compare" })
      .locator("select, button");
    const compareBoxes = await compareTargets.evaluateAll((elements: Element[]) =>
    ...
    const order = await page
      ...
      .first()
      .evaluateAll((elements: Element[]) => elements.map((element) => element.textContent));
    ...
    const boxes = await controls.evaluateAll((elements: Element[]) =>
      elements
        .map((element) => {
          const rect = element.getBoundingClientRect();
          return { width: rect.width, height: rect.height };
...
test("long content, forced colors, reduced motion, and non-color text preserve meaning", async ({
  page,
}) => {
  ...
  await page
    .getByText("Difference", { exact: true })
    .first()
  ...
  const comparisonTransition = await page
    .getByRole("form", { name: "Choose materials to compare" })
    .evaluate((element: Element) =>
  ...
  expect(
    await page
      .getByRole("button", { name: "Update comparison" })
...
test("tables expose captions and headers while decision states never rely on color alone", async ({
  page,
}) => {
  await page.goto(`${basePath}data/`);
  const table = page.getByRole("table");
  await expect(table).toHaveAccessibleName(
    `${explorer.materials.length} materials · ${explorer.groups[0]!.label}`,
  );
  await expect(table.getByRole("columnheader")).toHaveCount(
    explorer.groups[0]!.fieldKeys.length + 1,
  );
...
test("no-script and failed hydration preserve orientation, cautions, and safe onward links", async ({
  browser,
}) => {
  for (const route of ["compare", "data"] as const) {
    ...
    await expect(
      noScript.page.getByText("Named thermal tests are not directly interchangeable.", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      noScript.page.getByRole("link", { name: /methodology|comparison method/u }).first(),
    ).toHaveAttribute("href", new RegExp(`^${basePath.replaceAll("/", "\\/")}method/`, "u"));
    await noScript.context.close();

    const context = await browser.newContext();
    const page = await context.newPage();
    const component = new URL(
      islandComponentPath(route),
      `http://127.0.0.1:${mode === "root" ? 4321 : 4322}`,
    ).href;
    let componentAborted = false;
    await page.route(
      `**/${component.split("/").at(-1)}`,
      (intercepted: { abort(code?: string): Promise<void> }) => {
    ...
    await expect(
      page.getByRole("button", {
        name: route === "compare" ? "Update comparison" : "Clear filters",
      }),
    ).toBeVisible();
    await context.close();
  }
