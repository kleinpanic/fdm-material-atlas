...
function mapComponentUrl(): string {
  ...
  const island = [...html.matchAll(/<astro-island\b[^>]*\bcomponent-url="([^"]+)"/gu)]
    .find((match) => match[0].includes("MapExplorerIsland"));
...
async function openWithMapChunkAborted(browser: Browser): Promise<{ context: BrowserContext; page: Page }> {
  const context = await browser.newContext();
  const page = await context.newPage();
  const port = mode === "root" ? 4321 : 4322;
  const url = new URL(mapComponentUrl(), `http://127.0.0.1:${port}`).href;
  await page.route(url, (route: { abort(errorCode?: string): Promise<void> }) => route.abort("blockedbyclient"));
  await page.goto(mapPath());
  return { context, page };
}

test("built map exposes landmarks, structured alternatives, labels, and logical focus without SVG traps", async ({ page }) => {
  await openMap(page);
  ...
  await expect(page.getByRole("navigation", { name: "Explore visualization modes" }).getByRole("link")).toHaveCount(4);
  await expect(page.getByRole("navigation", { name: "Decision path index" }).getByRole("link")).toHaveCount(8);
  await expect(page.getByRole("navigation", { name: "Decision lane index" }).getByRole("link")).toHaveCount(8);
  await expect(page.getByRole("region", { name: "Lane by process-gate matrix" })).toContainText("Scroll horizontally");
  await expect(page.getByRole("region", { name: "Complete impact and flexibility material table" })).toContainText("Scroll horizontally");
...
test("axe passes the default, lane, thermal, and process-gate states", async ({ page }) => {
  ...
  await page.getByLabel("Named metric and method group", { exact: true }).selectOption(projection.thermalGroups[0]!.id);
  await axe(page);

  await page.getByLabel("Highlight a process gate").selectOption(projection.processGates.gates[0]!.id);
  await axe(page);
});

test("axe passes filtered impact states and the real-component omission/recovery fixture", async ({ page }) => {
  test.setTimeout(180_000);
  await openMap(page);
  ...
  await page.getByRole("button", { name: new RegExp(`^Highlight ${expert.material.name.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}\\.`, "u") }).last().click();
  ...
  await page.getByLabel("Find a material in the impact-flex view").fill("controlled zero result query");
  await axe(page);

  const fixture = renderMapOmissionRecoveryDocument(basePath);
  await page.setContent(fixture.html);
  await expect(page.getByRole("alert")).toContainText("The map view was reset because its previous state is no longer available.");
  await expect(page.getByText(fixture.omissionReason, { exact: false }).last()).toBeVisible();
  expect(await page.locator(`[data-material-id="${fixture.omittedMaterialId}"]`).count()).toBeGreaterThan(0);
  await axe(page);
});
...
test("no-script and failed hydration retain complete static meaning", async ({ browser }) => {
  ...
  await expect(noScript.page.getByRole("heading", { level: 1 })).toHaveText("Trace material choices through properties and process gates");
...
test("320px reflow, 200 percent zoom, and long labels avoid page overflow or clipping", async ({ page }) => {
  ...
  await longLabel.evaluate((element: Element) => {
    ...
    if (strong !== null) strong.textContent = "A deliberately long decision path label that must wrap without clipping any relevant words";
  });
  const longBox = await longLabel.evaluate((element: HTMLElement) => {
    const rect = element.getBoundingClientRect();
    const style = getComputedStyle(element);
    return { scrollWidth: element.scrollWidth, width: rect.width, overflow: style.overflow, overflowX: style.overflowX };
  });
...
test("forced colors and reduced motion preserve non-color meaning without position animation", async ({ page }) => {
  await page.emulateMedia({ forcedColors: "active", reducedMotion: "reduce" });
  await openMap(page);
  await page.getByRole("button", { name: `Highlight ${projection.lanes[0]!.label}` }).click();
  await expect(page.getByText("Selected decision lane", { exact: true }).first()).toBeVisible();
  await expect(page.getByLabel("Process-gate relationship legend")).toContainText("Applies — verify this gate");
  await expect(page.getByLabel("Process-gate relationship legend")).toContainText("Not listed for this lane");
  await expect(page.getByLabel("Impact-flex mark legend")).toContainText("Conditional — review conditions");
  const motion = await page.locator(".map-page *").evaluateAll((elements: Element[]) => elements.flatMap((element) => {
    const style = getComputedStyle(element);
    const duration = [...style.transitionDuration.split(","), ...style.animationDuration.split(",")]
      .map((value) => value.trim().endsWith("ms") ? Number.parseFloat(value) : Number.parseFloat(value) * 1000);
    const positional = /(?:transform|translate|top|right|bottom|left|inset)/u.test(style.transitionProperty);
    return Math.max(...duration, 0) > 1 || positional ? [{ duration: Math.max(...duration, 0), property: style.transitionProperty }] : [];
  }));
...
for (const viewport of [
  ...
  test(`map geometry is bounded and labels are complete at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    const labels = page.locator(".map-lane-directory a, .decision-lane-index a, [data-decision-lane] > header h3");
    for (let index = 0; index < await labels.count(); index += 1) {
      ...
      const result = await label.evaluate((element: HTMLElement) => {
        ...
        const style = getComputedStyle(element);
        return {
          contained: text.left >= own.left - 1 && text.right <= own.right + 1 && text.top >= own.top - 1 && text.bottom <= own.bottom + 1,
