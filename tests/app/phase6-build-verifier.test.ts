...
async function fixture() {
  ...
  for (const [name, base] of [
    ...
    const materialLinks = atlas.materials
      .map(({ slug }) => `<a href="${base}materials/${slug}/">${slug}</a>`)
      .join("");
    await writeFile(
      join(output, "index.html"),
      `<!doctype html><a href="${base}materials/">Atlas</a><a href="${base}method/#selector-scoring">Method</a>`,
    );
    await writeFile(
      join(output, "materials", "index.html"),
      `<!doctype html><astro-island props="{}"></astro-island><script src="${base}_astro/atlas.js"></script>${materialLinks}`,
    );
    await writeFile(
      join(output, "method", "index.html"),
      `<!doctype html>${[...methodFragments, "source-one", "method-one"].map((id) => `<section id="${id}"></section>`).join("")}<a href="${base}materials/material-1/#overview">Back</a>`,
    );
    for (const { slug } of atlas.materials) {
      ...
      await writeFile(
        ...
        `<!doctype html>${detailFragments.map((id) => `<section id="${id}"></section>`).join("")}<a href="${base}method/#source-one">Evidence</a>`,
      );
    }
  }
...
describe("Phase 6 emitted build verifier", () => {
  it("redirects the slashless repository base to its canonical trailing-slash URL", async () => {
    ...
    try {
      ...
      if (address === null || typeof address === "string")
        throw new Error("PREVIEW_ADDRESS_INVALID");
      const response = await fetch(`http://127.0.0.1:${address.port}/atlas-preview`, {
        redirect: "manual",
      });
  ...
  it("rejects a missing required detail fragment", async () => {
    ...
    await writeFile(
      path,
      (await readFile(path, "utf8")).replace('id="relationships"', 'id="relationship-missing"'),
    );
    expect(await codeFor(() => verifyPhase6Build({ ...input, runPublication: false }))).toBe(
      "PHASE6_DETAIL_FRAGMENT_MISSING",
    );
  });
  ...
  it("rejects dangling local fragments and scripts on reference routes", async () => {
    ...
    await writeFile(
      join(dangling.modes[0]!.output, "index.html"),
      '<!doctype html><a href="/method/#missing">Broken</a>',
    );
    expect(await codeFor(() => verifyPhase6Build({ ...dangling, runPublication: false }))).toBe(
      "PHASE6_LOCAL_FRAGMENT_MISSING",
    );
    ...
    expect(await codeFor(() => verifyPhase6Build({ ...scripted, runPublication: false }))).toBe(
      "PHASE6_STATIC_ROUTE_SCRIPT_FORBIDDEN",
    );
  });
  ...
  it("scans both emitted trees for exact private patterns without repository-history work", async () => {
    ...
    await writeFile(
      join(input.modes[1]!.output, "_astro/atlas.js"),
      'export const value="private-upstream-sentinel"',
    );
    expect(await codeFor(() => verifyPhase6Build({ ...input, sensitiveFile }))).toBe(
      "PHASE6_PUBLICATION_SCAN_FAILED",
    );
  });
});
