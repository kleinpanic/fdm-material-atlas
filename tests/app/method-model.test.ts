...
describe("method page model", () => {
  it("projects the complete ledger under stable contents and record anchors", () => {
    ...
    expect(model.methods.map(({ id }) => id)).toEqual(
      [...model.methods.map(({ id }) => id)].sort(),
    );
    expect(model.sources.map(({ id }) => id)).toEqual(
      [...model.sources.map(({ id }) => id)].sort(),
    );
  ...
  it("derives exact safe source actions, scopes, and material claim backlinks", () => {
    ...
    expect(
      model.sources.every(
        ({ claimUseCount, uses }) => claimUseCount === uses.length && uses.length > 0,
      ),
    ).toBe(true);
    expect(
      model.sources.every(
        ({ externalAction }) =>
          externalAction.url.startsWith("https://") &&
          externalAction.target === "_blank" &&
          externalAction.rel === "noopener noreferrer" &&
          externalAction.label === "Open external source",
      ),
    ).toBe(true);
  ...
  it("retains explanatory methods without fabricating claim uses", () => {
    ...
    expect(
      model.methods.every(
        ({ description, limitations }) => description.length > 0 && limitations.length > 0,
      ),
    ).toBe(true);
  });
  ...
  it("is invariant to public ledger and material permutation", () => {
    ...
    expect(buildMethodPageModel(atlas, "/docs/")).toEqual(
      buildMethodPageModel(loadPublicAtlas(), "/docs/"),
    );
  });
