...
function finalRegistrySource(): string {
  return `decisionMaps: Object.freeze([${lanes
    .map((lane) => `{ laneId: "${lane}", target: { id: "map" }, fragment: "${lane}" }`)
    .join(",")}])`;
}
  ...
  return {
    ...
    serviceGuidance: {
      ...
      records: materials.map((material) => ({
        material,
        fact: { state: "known", display: ["Known"] },
        evidence: { scopeLabels: ["Family guidance"], scopes: ["family-level-guidance"] },
    ...
    thermalGroups: Array.from({ length: 8 }, (_, index) => ({
      ...
      records: materials.map((material) => ({
        ...
        disposition: {
          disposition: "omitted",
          code: "no-observation-in-group",
          reason: "No observation in group",
        },
      })),
    })),
    processGates: {
      ...
      gates: gateRecords,
      relationships: lanes.flatMap((laneId, laneIndex) =>
        gateRecords.map((gate, gateIndex) => ({
          laneId,
          gateId: gate.id,
          relationship: laneIndex === gateIndex ? "applies" : "not-listed",
          label:
            laneIndex === gateIndex ? "Applies — verify this gate" : "Not listed for this lane",
        })),
      ),
    },
    impactFlex: {
      ...
      records: materials.map((material, slot) => ({
        material,
        impact: "high-impact",
        flexibility: "rigid",
        printDifficulty: "easy",
        impactFact: { state: "known", display: ["High impact"] },
        flexibilityFact: { state: "known", display: ["Rigid"] },
        printDifficultyFact: { state: "known", display: ["Easy"] },
        disposition: { disposition: "plotted" },
        slot,
        shape: "circle",
      })),
...
async function writeMode(root: string, base: string): Promise<void> {
  ...
  for (let index = 1; index <= 23; index += 1) {
    ...
    await writeFile(
      join(root, `materials/material-${index}/index.html`),
      `<!doctype html><a href="${prefix}/map/">Map</a>`,
    );
  }
  ...
  await writeFile(
    ...
    `<!doctype html><link rel="canonical" href="https://atlas.example${base}"><astro-island component-url="${prefix}/_astro/selector.js" component-export="SelectorIsland" renderer-url="${prefix}/_astro/client.js" props='${JSON.stringify({ model: selectorProjection })}' ssr client="load"><p>Decision map is not available yet</p></astro-island>`,
  );
  const fragmentTargets = [
    "main-content",
    ...modes,
    ...lanes,
    ...projection(base).processGates.gates.map(({ id }) => id),
  ]
    .map((id) => `<span id="${id}"></span>`)
    .join("");
  await writeFile(
    join(root, "map/index.html"),
    `<!doctype html><link rel="canonical" href="https://atlas.example${prefix}/map/"><nav>${modes.map((mode) => `<a href="${prefix}/map/#${mode}">${mode}</a>`).join("")}${lanes.map((lane) => `<a href="${prefix}/map/#${lane}">${lane}</a>`).join("")}</nav>${fragmentTargets}${island(base)}`,
  );
  for (const route of ["method", "materials", "compare", "data"])
    await writeFile(join(root, `${route}/index.html`), `<!doctype html><a href="${base}">Home</a>`);
  ...
  await writeFile(
    join(root, "_astro/map.js"),
    "import './shared.js'; export const MapExplorerIsland = true;",
  );
...
describe("Phase 8 emitted build verifier", () => {
  it("accepts only the exact explicit eight-lane final registry", () => {
    ...
    expect(() =>
      assertRegistryStage(
        registrySource.replace(`laneId: "${lanes[0]}"`, `laneId: "${lanes[1]}"`),
        "final",
      ),
    ).toThrow("REGISTRY_ACTIVATION_MISSING");
    expect(() => assertRegistryStage(`${registrySource}\nallDecisionMaps: true`, "final")).toThrow(
      "REGISTRY_ACTIVATION_MISSING",
    );
  });
  ...
  it("requires a matching pre-activation receipt and changed final artifacts", () => {
    ...
    const common = {
      ...
      digests: {
        route: "a".repeat(64),
        fragments: "b".repeat(64),
        projectionContract: "c".repeat(64),
      },
    ...
    expect(() => assertPreactivationReceipt({ ...prior, stage: "final" }, current)).toThrow(
      "PREACTIVATION_RECEIPT_INVALID",
    );
    expect(() =>
      assertPreactivationReceipt(
        {
  ...
  it("accepts complete, scoped, pre-activation artifacts in both deployment bases", async () => {
    ...
    expect(
      report.modes.every(
        ({ projectionGzipBytes, totalGzipBytes }) => projectionGzipBytes > 0 && totalGzipBytes > 0,
      ),
    ).toBe(true);
  });
  ...
  it.each([
    [
      ...
      async (outputs: Awaited<ReturnType<typeof fixture>>) =>
        writeFile(join(outputs.root, "_astro/map.js.map"), "{}"),
    ],
    [
      "MAP_FRAGMENT_MISSING",
      async (outputs: Awaited<ReturnType<typeof fixture>>) => {
    ...
    [
      ...
      async (outputs: Awaited<ReturnType<typeof fixture>>) =>
        writeFile(join(outputs.root, "_astro/map.js"), "fetch('/map-data.json')"),
    ],
    [
      "ROUTE_SCOPE_VIOLATION",
      async (outputs: Awaited<ReturnType<typeof fixture>>) =>
        writeFile(
          join(outputs.root, "_astro/selector.js"),
          "import './map.js'; export const SelectorIsland = true;",
        ),
    ],
    [
      "SELECTOR_ACTIVATED_TOO_EARLY",
      async (outputs: Awaited<ReturnType<typeof fixture>>) => {
        const path = join(outputs.root, "index.html");
        const html = await (await import("node:fs/promises")).readFile(path, "utf8");
        await writeFile(
          path,
          html.replaceAll("Decision map is not available yet", "Open material decision map"),
        );
      },
    ],
    [
      ...
      async (outputs: Awaited<ReturnType<typeof fixture>>) =>
        writeFile(
          join(outputs.repository, "_astro/map.js"),
  ...
  it.each(["", "preactivation", "final "])(
    "rejects an invalid stage token without normalization",
    async (stage) => {
      const outputs = await fixture();
      await expect(
        verifyPhase8Build({
      ...
      ).rejects.toMatchObject({ code: "STAGE_INVALID" });
    },
  );
});
