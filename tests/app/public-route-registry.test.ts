...
function completedRegistry(): PublicRouteRegistry {
  return Object.freeze({
    ...
    decisionMaps: Object.freeze([
      Object.freeze({ laneId: LANE_ID, target: { id: "map" as const }, fragment: LANE_ID, verifiedFragments: VERIFIED_MAP_FRAGMENTS }),
    ]),
...
describe("public selector route registry", () => {
  ...
  it.each([
    ...
    for (const route of model.routes.materials) {
      ...
      expect(route.decisionMaps.every(({ laneId, action }) =>
  ...
  it.each([
    ...
    ["ROUTE_REGISTRY_LANE_UNKNOWN", () => ({ ...completedRegistry(), decisionMaps: [{ laneId: "lane-unknown" as DecisionLaneId, target: { id: "map" }, fragment: "lane-easy-prototypes", verifiedFragments: VERIFIED_MAP_FRAGMENTS }] })],
    ["ROUTE_REGISTRY_TARGET_MISMATCH", () => ({ ...completedRegistry(), decisionMaps: [{ laneId: LANE_ID, target: { id: "home" }, fragment: LANE_ID, verifiedFragments: VERIFIED_MAP_FRAGMENTS }] })],
    ["ROUTE_REGISTRY_TARGET_MISMATCH", () => ({ ...completedRegistry(), decisionMaps: [{ laneId: LANE_ID, target: { id: "map" }, fragment: "lane-outdoor", verifiedFragments: VERIFIED_MAP_FRAGMENTS }] })],
