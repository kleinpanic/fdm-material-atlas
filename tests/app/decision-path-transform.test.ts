...
describe("complete decision-path transform", () => {
  it("derives exactly eight four-stage paths in canonical order", () => {
    ...
    expect(result.every(({ need, propertyChecks, verification }) =>
      need.length > 0 && propertyChecks.length > 0 && verification.length > 0)).toBe(true);
    ...
    for (const lane of result) {
      ...
      expect(lane.propertyChecks.every(({ field, label }) => field.length > 0 && label.length > 0)).toBe(true);
      expect(lane.candidates.every(({ id, href }) => materialIds.has(id) && href.startsWith("/atlas-preview/materials/"))).toBe(true);
      expect(lane.processGates.every(({ id, href, capabilityLabel }) =>
        gateIds.has(id) && href === `/atlas-preview/map/#${id}` && capabilityLabel.length > 0)).toBe(true);
    }
  });
  ...
  it("keeps zero, one, nine, and thirteen candidate boundaries explicit", () => {
    ...
    const outdoorIds = buildDecisionPaths(baseline)
      .find(({ id }) => id === "lane-outdoor")!.candidates.map(({ id }) => id);
  ...
  it("retains indeterminate material IDs outside the candidate split", () => {
    ...
    material.properties.moistureSensitivity.value = { state: "unknown", reason: "Synthetic unknown." };
  ...
  it("is invariant to every relevant canonical registry permutation", () => {
    expect(buildDecisionPaths(reverseRegistries(cloneAtlas()), "/repo/"))
      .toEqual(buildDecisionPaths(loadPublicAtlas(), "/repo/"));
  });

  it.each([
    ["DECISION_PATH_LANE_DUPLICATE", (atlas: AtlasV1) => {
      atlas.decisionLanes[1] = structuredClone(atlas.decisionLanes[0]!);
    }],
    ["DECISION_PATH_LANE_MISSING", (atlas: AtlasV1) => {
    ...
    ["DECISION_PATH_VISUALIZATION_MISSING", (atlas: AtlasV1) => {
      atlas.visualizationReferences = atlas.visualizationReferences.filter(({ kind }) => kind !== "decision-path");
    }],
    ["DECISION_PATH_PROPERTY_LABEL_MISSING", (atlas: AtlasV1) => {
