...
async function site(
  routes: Record<string, { type: string; body: string; status?: number; location?: string }>,
) {
...
describe("bounded live surface capture", () => {
  it("captures required routes and recursively referenced same-origin assets", async () => {
    const origin = await site({
      "/atlas/": {
        type: "text/html",
        body: document(
          '<a href="/atlas/materials/pla/">PLA</a><a href="https://github.com/example/project">Source</a>',
        ),
      },
  ...
  it.each([
    [
      ...
      {
        "/atlas/": { type: "text/html", body: "", status: 302, location: "https://example.test/" },
      },
      "LIVE_REDIRECT_ORIGIN",
    ],
    [
      "source map",
      { "/atlas/_astro/app.12345678.css.map": { type: "application/json", body: "{}" } },
      "LIVE_SOURCE_MAP",
    ],
  ])("rejects %s", async (_label, override, code) => {
    const routes: Record<
      string,
      { type: string; body: string; status?: number; location?: string }
    > = {
