...
describe("authenticated established GitHub target", () => {
  ...
  it.each([
    ["missing auth", (value: ReturnType<typeof repository>) => (value.auth.logins = []), "GITHUB_AUTH_INVALID"],
    ["ambiguous auth", (value: ReturnType<typeof repository>) => value.auth.logins.push("other"), "GITHUB_AUTH_INVALID"],
    ["owner", (value: ReturnType<typeof repository>) => (value.repository.owner.login = "rejected-value"), "GITHUB_TARGET_MISMATCH"],
    ["name", (value: ReturnType<typeof repository>) => (value.repository.name = "rejected-value"), "GITHUB_TARGET_MISMATCH"],
    ["visibility", (value: ReturnType<typeof repository>) => (value.repository.visibility = "PRIVATE"), "GITHUB_REPOSITORY_SETTINGS_INVALID"],
    ["permission", (value: ReturnType<typeof repository>) => (value.repository.viewerPermission = "WRITE"), "GITHUB_REPOSITORY_SETTINGS_INVALID"],
    ["branch", (value: ReturnType<typeof repository>) => (value.repository.defaultBranchRef.name = "trunk"), "GITHUB_REPOSITORY_SETTINGS_INVALID"],
    ["origin", (value: ReturnType<typeof repository>) => (value.origin = "https://example.test/rejected-value"), "GITHUB_ORIGIN_INVALID"],
    ["Pages mode", (value: ReturnType<typeof repository>) => (value.pages.buildType = "legacy"), "GITHUB_PAGES_SETTINGS_INVALID"],
    ["HTTPS", (value: ReturnType<typeof repository>) => (value.pages.httpsEnforced = false), "GITHUB_PAGES_SETTINGS_INVALID"],
    ["remote advance", (value: ReturnType<typeof repository>) => (value.refs[0]!.sha = "6".repeat(40)), "GITHUB_REMOTE_MAIN_CHANGED"],
    ["divergence", (value: ReturnType<typeof repository>) => (value.relation = "diverged"), "GITHUB_CANDIDATE_DIVERGED"],
    ["unknown ref", (value: ReturnType<typeof repository>) => value.refs.push({ name: "refs/heads/release", sha: PRIOR }), "GITHUB_REF_TOPOLOGY_INVALID"],
...
describe("existing repository publication transition", () => {
  ...
  it("accepts an equality no-op", () => {
    const noOpExpected = { ...expected, priorRemoteMainSha: CANDIDATE };
  ...
  it("rejects unrelated refs, dependency heads, pull heads, and invalid merge synthesis", () => {
    for (const mutate of [
      (value: ReturnType<typeof repository>) => value.refs.push({ name: "refs/tags/v1", sha: CANDIDATE }),
...
function runEvidence(workflowPath = ".github/workflows/pages.yml") {
  ...
  const jobs = (isPages ? ["build", "deploy", "probe"] : ["quality", "build", "browser", "performance"])
    .map((name, index) => ({ id: 100 + index, name, runId: 41, attempt: 2, status: "completed", conclusion: "success", environment: name === "deploy" ? "github-pages" : null }));
  return {
    workflow: { id: isPages ? 12 : 11, path: workflowPath, state: "active" },
    run: { id: 41, workflowId: isPages ? 12 : 11, event: "push", branch: "main", ref: "refs/heads/main", sha: CANDIDATE, attempt: 2, status: "completed", conclusion: "success" },
    jobs,
    artifact: isPages ? { id: 71, name: "github-pages", digest: `sha256:${"a".repeat(64)}`, runId: 41, attempt: 2, producerJobId: 100 } : null,
    deployment: isPages ? { id: 81, runId: 41, attempt: 2, environment: "github-pages", sha: CANDIDATE, ref: "refs/heads/main", consumerJobId: 101, status: "success" } : null,
  };
}
...
describe("exact workflow, artifact, and Pages evidence", () => {
  ...
  it.each([
    ...
    ["wrong event", (value: ReturnType<typeof runEvidence>) => (value.run.event = "workflow_dispatch")],
    ["wrong ref", (value: ReturnType<typeof runEvidence>) => (value.run.ref = "refs/heads/release")],
    ...
    ["extra job", (value: ReturnType<typeof runEvidence>) => value.jobs.push({ ...value.jobs[0]!, id: 999, name: "extra" })],
    ["skipped job", (value: ReturnType<typeof runEvidence>) => (value.jobs[0]!.conclusion = "skipped")],
    ["wrong producer", (value: ReturnType<typeof runEvidence>) => (value.artifact!.producerJobId = 102)],
    ["wrong consumer", (value: ReturnType<typeof runEvidence>) => (value.deployment!.consumerJobId = 100)],
    ...
    expect(() =>
      verifyGitHubRelease({
        stage: "run",
        expected: { ...expected, workflowPath: ".github/workflows/pages.yml", jobNames: ["build", "deploy", "probe"] },
        evidence,
      }),
    ).toThrow();
  });
...
describe("fixed read-only process seam", () => {
  it("uses only explicit gh/git reads with scrubbed environment and bounded parsing", async () => {
    ...
    const outputs = [
      ...
      { stdout: refs().map((ref) => `${ref.sha}\t${ref.name}`).join("\n") },
      { stdout: "" },
    ];
    const run = async (file: string, args: string[], options: { env: Record<string, string | undefined> }) => {
    ...
    expect(calls.map((call) => call.file)).toEqual(["gh", "gh", "gh", "git", "git"]);
    expect(calls.every((call) => !Object.keys(call.env).some((key) => /TOKEN|SECRET|GOOGLE|GOG/u.test(key)))).toBe(true);
    expect(calls.some((call) => call.args.includes("create") || call.args.includes("delete") || call.args.includes("edit"))).toBe(false);
  });
});
