...
describe("decision path renderer", () => {
  ...
  it("dispatches the same action from pointer marks and keyboard-owned controls", () => {
    ...
    const control = findNode(
      controlTree,
      (node) =>
        node.props["data-candidate-control"] === true &&
        node.props["data-material-id"] === candidate.id,
    );
    ...
    const mark = findNode(
      markTree,
      (node) =>
        node.props["data-candidate-mark"] === true &&
        node.props["data-material-id"] === candidate.id,
    );
  ...
  it("keeps candidate previews equivalent and source projection-only", () => {
    ...
    expect(source).not.toMatch(
      /fetch\s*\(|XMLHttpRequest|localStorage|sessionStorage|Atlas|querySelector|focus\s*\(|dangerouslySetInnerHTML/iu,
    );
  ...
  it("uses preview only for visual emphasis until a candidate is activated", () => {
    ...
    expect(html).toContain(
      `data-material-id="${candidate.id}" aria-pressed="false" data-previewed="true"`,
    );
...
describe("thermal guidance renderer", () => {
  it("places the exact caution before complete practical-service controls and rows", () => {
    ...
    for (const record of projection.serviceGuidance.records)
  ...
  it("uses identical material actions for the service diagram and ordered HTML controls", () => {
    ...
    const control = findNode(
      controlTree,
      (node) =>
        node.props["data-service-control"] === true &&
        node.props["data-material-id"] === record.material.id,
    )!;
    (control.props.onClick as () => void)();
    ...
    const mark = findNode(
      markTree,
      (node) =>
        node.props["data-service-mark"] === true &&
        node.props["data-material-id"] === record.material.id,
    )!;
    (mark.props.onClick as () => void)();
  ...
  it("dispatches closed view, group, search, sort, and reset actions from native controls", () => {
    ...
    const namedRadio = findNode(
      tree,
      (node) => node.props["data-thermal-view"] === "named-observations",
    )!;
    const search = findNode(
      tree,
      (node) => node.props["aria-label"] === "Find a material in this thermal view",
    )!;
    ...
    (namedRadio.props.onChange as (event: { currentTarget: { checked: boolean } }) => void)({
      currentTarget: { checked: true },
    });
    (search.props.onInput as (event: { currentTarget: { value: string } }) => void)({
      currentTarget: { value: "PLA" },
    });
    (sort.props.onChange as (event: { currentTarget: { value: string } }) => void)({
      currentTarget: { value: "low-endpoint" },
    });
    ...
    const state = namedState.reduce(
      createMapReducer(projection),
      createInitialMapState(projection),
    );
    ...
    const groupSelect = findNode(
      namedTree,
      (node) => node.props["aria-label"] === "Named metric and method group",
    )!;
    ...
    expect(groupActions).toEqual([
      {
        ...
        mode: "thermal-ranges",
        groupId: projection.thermalGroups[0]!.id,
      },
    ]);
  });
  ...
  it("keeps the two thermal concepts in separate accessible trees and stays source-only", () => {
    ...
    expect(source).not.toMatch(/href\s*=\s*\{?\s*[`'"]\//u);
    expect(source).not.toMatch(
      /generic heat|heat resistance|heat score|numeric sort|average|combined (?:axis|scale)/iu,
    );
