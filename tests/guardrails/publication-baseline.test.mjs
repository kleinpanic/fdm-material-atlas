...
test("clean committed and unborn repositories pass every default surface without mutation", () => {
  for (const commitFixture of [false, true]) {
    ...
    assert.deepEqual(
      ...
      ["repository", "working", "tracked", "history"],
    );
...
test("repeated artifact options scan all directories and missing artifacts fail closed", () => {
  ...
  const dirty = runBaseline(child, ["--artifact", cleanArtifact, "--artifact", dirtyArtifact], {
    PUBLICATION_SENSITIVE_PATTERNS_JSON: JSON.stringify([marker]),
  });
  assertFailedWith(dirty, "private-source-pattern", [marker, token, unsafeSegment, dirtyArtifact]);
  assert.equal(
    parseOutput(dirty).surfaces.filter(({ surface }) => surface === "artifact").length,
    2,
  );
  const artifactSurfaces = parseOutput(dirty).surfaces.filter(
    ({ surface }) => surface === "artifact",
  );
  assert.deepEqual(
    artifactSurfaces.map(({ artifactOrdinal }) => artifactOrdinal),
    [0, 1],
  );
  for (const surface of artifactSurfaces)
    assert.match(surface.artifactDigest, /^sha256:[a-f0-9]{64}$/);
  ...
  assertFailedWith(
    runBaseline(child, ["--artifact", cleanArtifact, "--artifact", missing]),
    "surface-inspection-failed",
    [missingSegment, missing],
  );
});
