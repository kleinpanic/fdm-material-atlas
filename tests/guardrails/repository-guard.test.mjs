...
test("accepts an unborn independent nested repository without exposing identity", async () => {
  ...
  const inspection = await assertRepository({
    cwd: child,
    expectedRoot: child,
    remotePolicy: "absent",
  });
...
test("exported inspection normalizes filesystem and corrupt-Git failures", async () => {
  ...
  await expectRule(
    inspectRepository({ cwd: missing, expectedRoot: missing }),
...
test("rejects Git files, linked worktrees, and symlinked external common directories", async () => {
  ...
  await expectRule(
    assertRepository({
      cwd: gitFileFixture.child,
      expectedRoot: gitFileFixture.child,
      remotePolicy: "absent",
    }),
  ...
  await expectRule(
    assertRepository({
      cwd: symlinkFixture.child,
      expectedRoot: symlinkFixture.child,
      remotePolicy: "absent",
    }),
    "common-directory-mismatch",
  );
...
test("rejects shallow and promisor repositories before auditing history", async () => {
  ...
  git(sourceFixture.fixtureRoot, [
    "clone",
    "--depth",
    "1",
    `file://${sourceFixture.child}`,
    shallow,
  ]);
  ...
  await expectRule(
    assertRepository({
      cwd: partialFixture.child,
      expectedRoot: partialFixture.child,
      remotePolicy: "absent",
    }),
    "history-incomplete",
  );
