...
function assertRedacted(value, forbidden = []) {
  ...
  for (const finding of findings) {
    ...
    assert.deepEqual(
      ...
      Object.keys(finding)
        .filter((key) => ["objectType", "ruleId", "safeLocation", "surface"].includes(key))
        .sort(),
    );
  }
}
...
test("all four modes accept clean synthetic surfaces, including unborn history", async () => {
  ...
  const artifactReport = await scanPublication({
    root,
    mode: "artifact",
...
test("the public environment example is path-allowed but remains content-scanned", async () => {
  ...
  const policy = await loadPublicationPolicy({ root, env: {} });
  assert.equal(
    scanPath(".env.example", { policy, surface: "working", objectType: "file" }).some(
      ({ ruleId }) => ruleId === "operational-path",
    ),
    false,
  );
...
test("canonical model-operation path classes are ignored and rejected on every public surface", async () => {
  const [{ PROHIBITED_PATH_CLASSES }, { loadPublicationPolicy, scanPath, scanPublication }] =
    await Promise.all([import(PROHIBITED_PATHS_URL), loadInterfaces()]);
  const publicIgnore = readFileSync(new URL("../../.gitignore", import.meta.url), "utf8").split(
    /\r?\n/,
  );
  for (const { ignore } of PROHIBITED_PATH_CLASSES)
    assert.ok(publicIgnore.includes(ignore), ignore);

  const root = createRepository();
  const policy = await loadPublicationPolicy({ root, env: {} });
  for (const { fixture } of PROHIBITED_PATH_CLASSES)
    write(root, fixture, "synthetic operating fixture\n");
  const working = await scanPublication({ root, mode: "working", policy });
  assert.ok(
    working.findings.filter(({ ruleId }) => ruleId === "operational-path").length >=
  ...
  for (const mode of ["tracked", "history"]) {
    const report = await scanPublication({ root, mode, policy });
    assert.ok(
      report.findings.filter(({ ruleId }) => ruleId === "operational-path").length >=
        PROHIBITED_PATH_CLASSES.length,
      mode,
    );
  }
  ...
  for (const { fixture } of PROHIBITED_PATH_CLASSES)
    write(artifact, fixture, "synthetic operating fixture\n");
  const artifactReport = await scanPublication({
    root,
    mode: "artifact",
    artifactPath: artifact,
  ...
  for (const { fixture } of PROHIBITED_PATH_CLASSES) {
    ...
    assert.ok(
      refFindings.some(({ ruleId }) => ruleId === "operational-path"),
      fixture,
    );
  }
  ...
  assert.ok(
    refs.findings.some(
      ({ ruleId, objectType }) => ruleId === "operational-path" && objectType === "ref",
    ),
  );
});
...
test("environment exact patterns are detected and redacted across every mode", async () => {
  ...
  for (const mode of ["tracked", "history"]) {
    const report = await scanPublication({ root, mode, policy });
    assert.ok(
      report.findings.some(({ ruleId }) => ruleId === "private-source-pattern"),
      mode,
    );
  ...
  const artifact = await scanPublication({
    root,
    mode: "artifact",
...
test("sensitive files obey the outside-root or ignored, unstaged, untracked invariant", async () => {
  ...
  const outside = write(
    makeTemp(join(tmpdir(), "publication-outside-")),
    "patterns.json",
...
test("default sensitive input is optional only when absent and rejects unusable entries", async () => {
  ...
  for (const kind of ["directory", "symlink"]) {
    ...
    if (kind === "directory") {
      ...
      const outside = write(
        makeTemp(join(tmpdir(), "publication-link-target-")),
        "patterns.json",
        JSON.stringify(["safe"]),
      );
...
test("environment and explicit sensitive files merge without ambient precedence", async () => {
  ...
  assert.equal(
    report.findings.filter(({ ruleId }) => ruleId === "private-source-pattern").length,
    1,
  );
...
test("ignored-file exact patterns are detected in every surface", async () => {
  ...
  const policy = await loadPublicationPolicy({
    root,
    env: {},
    sensitiveFile: join(root, ".publication-sensitive-patterns"),
  });
...
test("current GitHub, Google, session, and encrypted-key credential shapes are detected", async () => {
  ...
  for (const value of additionalCredentialShapes()) {
    ...
    assert.ok(
      findings.some(({ ruleId }) => ruleId === "credential-signature"),
      value.slice(0, 8),
    );
...
test("unsafe source maps and generated metadata are rejected in tracked and artifact modes", async () => {
  ...
  write(
    root,
    "bundle.js.map",
    JSON.stringify({ version: 3, sourcesContent: ["synthetic source"] }),
  );
  ...
  write(
    artifact,
    "manifest.json",
    JSON.stringify({ sourcesContent: ["synthetic embedded source"] }),
  );
  write(
    artifact,
    "_astro/stats.json",
    JSON.stringify({ output: "file:///synthetic/workspace/app.js" }),
  );
  const built = await scanPublication({ root, mode: "artifact", artifactPath: artifact, policy });
  assert.equal(
...
test("inline and external source-map directives are rejected on every stored-code surface", async () => {
  ...
  for (const mode of ["tracked", "history"]) {
    const report = await scanPublication({ root, mode, policy });
    assert.ok(
      report.findings.some(({ ruleId }) => ruleId === "unsafe-source-map"),
      mode,
    );
  }
...
test("source-map directives in HTML and SVG containers are rejected on all stored surfaces", async () => {
  ...
  for (const mode of ["tracked", "history"]) {
    const report = await scanPublication({ root, mode, policy });
    assert.equal(
      report.findings.filter(({ ruleId }) => ruleId === "unsafe-source-map").length,
      2,
      mode,
    );
  }
...
test("history scans branch, tag, note, and unusual ref names without disclosure", async () => {
  ...
  assert.ok(
    report.findings.some(
      ({ ruleId, objectType }) => ruleId === "private-source-pattern" && objectType === "ref",
    ),
  );
  assertRedacted(report, [marker]);
});
...
test("tracked and history scans preserve symlink and gitlink modes", async () => {
  ...
  for (const mode of ["tracked", "history"]) {
    const report = await scanPublication({ root, mode, policy });
    assert.ok(
      report.findings.some(({ ruleId }) => ruleId === "unsafe-symlink"),
      mode,
    );
    assert.ok(
      report.findings.some(({ ruleId }) => ruleId === "unsafe-gitlink"),
      mode,
    );
...
test("malformed input and missing, unreadable, or escaping surfaces fail closed and redact errors", async () => {
  ...
  const symlinkReport = await scanPublication({
    root,
    mode: "artifact",
    artifactPath: artifact,
    policy,
...
test("stable file reader exposes a controlled failure seam without privilege assumptions", async () => {
  ...
  await assert.rejects(
    readStableFile(marker, {
      openFile: async () => {
        throw new Error(`uncontrolled ${marker}`);
      },
    }),
    (error) => {
...
test("stable file reader bounds growth and detects same-size metadata replacement", async () => {
  ...
  await assert.rejects(
    readStableFile("synthetic-replacement", {
      maximumBytes: 8,
      openFile: async () => replacedHandle,
    }),
...
test("CLI returns nonzero with redacted stdout and stderr", async () => {
  ...
  const result = spawnSync(
    process.execPath,
    [SCANNER_URL.pathname, "--mode", "working", "--root", root],
    {
      encoding: "utf8",
      env: {
        ...process.env,
