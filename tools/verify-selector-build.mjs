...
function parseProps(serialized) {
  try {
    ...
    const looksEncoded = Object.values(parsed).some(
...
function inspectPropBoundary(value, exactPatterns, seen = new Set()) {
  if (typeof value === "string") {
    if (exactPatterns.some((pattern) => pattern !== "" && value.includes(pattern)))
      fail("SELECTOR_PRIVATE_PATTERN_FORBIDDEN");
    return;
  }
...
function decodeCompactPageModel(value) {
  ...
  if (
    dictionary.length > 16_384 ||
    dictionary.some(
      (entry, index) =>
        typeof entry !== "string" ||
        entry.length > 4_096 ||
        (index > 0 && entry <= dictionary[index - 1]),
    )
  ) {
    fail("SELECTOR_PROPS_SHAPE_INVALID");
  }
  ...
  function decode(node, depth = 0) {
    if (++nodes > 200_000 || depth > 64 || !Array.isArray(node) || node.length === 0)
      fail("SELECTOR_PROPS_SHAPE_INVALID");
    if (node[0] === 0) {
      if (
        node.length !== 2 ||
    ...
    if (node[0] === 3) {
      if (node.length !== 2 || typeof node[1] !== "number" || !Number.isFinite(node[1]))
        fail("SELECTOR_PROPS_SHAPE_INVALID");
      return node[1];
    }
    if (node[0] === 4) {
      if (node.length !== 2 || (node[1] !== 0 && node[1] !== 1))
        fail("SELECTOR_PROPS_SHAPE_INVALID");
      return node[1] === 1;
    }
    if (node[0] === 5) {
    ...
    for (let index = 1; index < node.length; index += 2) {
      ...
      if (!Number.isInteger(keyIndex) || keyIndex < 0 || keyIndex >= dictionary.length)
        fail("SELECTOR_PROPS_SHAPE_INVALID");
...
function validatePageModelShape(props) {
  ...
  if (
    ...
    !exactKeys(display, ["materials"]) ||
    !Array.isArray(display.materials) ||
    typeof routes !== "object" ||
    routes === null ||
    !Array.isArray(routes.materials)
  )
    fail("SELECTOR_PROPS_SHAPE_INVALID");
  ...
  if (
    ...
    criterionIds.some(
      (id) =>
        defaults[id] !==
        projection.criteria.find((criterion) => criterion.id === id)?.defaultOptionId,
    ) ||
  ...
  async function walk(directory) {
    ...
    for await (const entry of stream) {
      ...
      if (info.isDirectory()) await walk(path);
        ...
        if (info.size > MAX_FILE_BYTES || files.size >= MAX_FILES)
          fail("SELECTOR_OUTPUT_LIMIT_EXCEEDED");
...
function fileForPublicUrl(raw, base, files, code = "SELECTOR_CLIENT_REFERENCE_INVALID") {
  ...
  if (
    ...
    (base !== "/" && url.pathname.startsWith(`${base}${base.slice(1)}`))
  )
    fail(code);
  let logical;
  try {
...
function resolveImport(specifier, importer, files) {
  ...
  if (candidate.startsWith("../") || candidate === ".." || !files.has(candidate))
    fail("SELECTOR_CLIENT_REFERENCE_INVALID");
  return candidate;
}
...
async function validateRoutes(props, base, files) {
  ...
  for (const record of records) {
    ...
    if (fragment !== "") {
      const html = await readFile(files.get(candidate).path, "utf8").catch(() =>
        fail("SELECTOR_LINK_HREF_INVALID"),
      );
      const escaped = fragment.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const count = [...html.matchAll(new RegExp(`(?:^|\\s)id=(?:"${escaped}"|'${escaped}')`, "g"))]
        .length;
...
export async function verifySelectorBuild({
  ...
  if (
    ...
    !/^\/(?:[a-z0-9-]+\/)*$/.test(base) ||
    [maxGzipBytes, maxIndexHtmlBytes, maxSelectorEntryJavaScriptBytes].some(
      (cap) => !Number.isInteger(cap) || cap < 1,
    )
  ) {
  ...
  if ([...files.keys()].some((name) => name.endsWith(".map")))
    fail("SELECTOR_SOURCE_MAP_FORBIDDEN");
  ...
  const islands = [];
  const selectorHtmlBytes = await readFile(files.get("index.html")?.path).catch(() =>
    fail("SELECTOR_OUTPUT_INVALID"),
  );
  ...
  if (islands.length !== 1 || islands[0].name !== "index.html")
    fail("SELECTOR_ISLAND_COUNT_INVALID", { islandCount: islands.length });
  ...
  if (!selectorEntry.endsWith(".js") && !selectorEntry.endsWith(".mjs"))
    fail("SELECTOR_CLIENT_REFERENCE_INVALID");
  ...
  if (selectorEntryJavaScriptBytes > maxSelectorEntryJavaScriptBytes) {
    fail("SELECTOR_ENTRY_JAVASCRIPT_BUDGET_EXCEEDED", {
  ...
  for (const match of homeHtml.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi)) {
    ...
    inlineScriptCount += 1;
    if (PROHIBITED_CLIENT_TEXT.some((pattern) => pattern.test(source)))
      fail("SELECTOR_CLIENT_IMPORT_FORBIDDEN");
    if (FETCH_PATTERN.test(source)) fail("SELECTOR_RUNTIME_FETCH_FORBIDDEN");
    if (prohibitedExactPatterns.some((pattern) => pattern !== "" && source.includes(pattern)))
      fail("SELECTOR_PRIVATE_PATTERN_FORBIDDEN");
  ...
  while (pending.length > 0) {
    ...
    const bytes = await readFile(files.get(name).path).catch(() =>
      fail("SELECTOR_CLIENT_REFERENCE_INVALID"),
    );
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!value || !["--output", "--base"].includes(flag) || Object.hasOwn(values, flag))
      fail("SELECTOR_ARGUMENTS_INVALID");
    values[flag] = value;
  }
...
async function main() {
  ...
  const report = await verifySelectorBuild({
    outputRoot: resolve(PROJECT_ROOT, args["--output"]),
    base: args["--base"],
  });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    const safe =
      error instanceof SelectorBuildError
        ? error.toJSON()
        : { code: "SELECTOR_VERIFICATION_FAILED" };
